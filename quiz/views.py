...
def single_question(request, quiz_id, question_id):
    quiz = get_object_or_404(Quiz, pk=quiz_id)
    question_total = quiz.question_set.all()
    ...
    try:
        res=Result.objects.get(quiz=quiz,user=request.user)
    except Result.DoesNotExist:
        res=False
    print(res)
...
def results(request, quiz_id):
    ...
    total_questions = Quiz.objects.get(pk=quiz_id).num_questions

    # formats accuracy as % with no decimal digits
    accuracy = num_correct/(total_questions)

    accuracy_con = (num_correct/(total_questions))*100.00
    accuracy_over_75 = False
    if accuracy >= .75:
        accuracy_over_75 = True

    accuracy_formatted = "{:.0%}".format(accuracy)
    b = Result(quiz=quiz ,user=request.user,score=accuracy_con)
    b.save()
    context = {
...
def create_quiz(request,key):
    ...
    if request.method == 'POST':
        ...
        new_quiz = Quiz(quiz_for=quiz_for,quiz_title=quiz_name, num_questions=num_questions)
        new_quiz.save()
